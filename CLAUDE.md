...
print(TestClient(main.app).get('/about').status_code)"
```
...
- `main.py` — FastAPI ルーティングと SSR 描画。ルートは `/`（表示）/ `/about` / `/propose` / `/feedback/{log_id}` / `/ingest` / `/delete/{log_id}` / `/essence/{key}`。**全 POST は PRG パターン**で `/` へ 303 リダイレクト（リロードによる提案/学習の二重実行を防ぐ）。flash メッセージはクエリ文字列で次の GET に 1 回だけ引き継ぐ。
...
- `templates/index.html` — 1 画面の本体（提案・フィードバック・学習・好みの表示/編集）。`templates/about.html` — 使い方・仕組みの説明画面（`/about`）。

### 「我が家の好み」のカテゴリは ESSENCE_KEYS が単一の真実

`gemini_service.ESSENCE_KEYS`（`flavor_preference` / `disliked_ingredients` / `favorite_menus` / `presentation_preference`）が好みカテゴリの定義。プロンプトの JSON 雛形・カテゴリ一覧・`main.py` の編集バリデーション（`/essence/{key}`）がすべてこの辞書から導出される。カテゴリを増減するときはここだけを変更する。`presentation_preference` は「味」ではなく提案の伝え方・出力形式の好みを保持する点に注意。
...
- `/delete/{log_id}` はログ（と紐づく summary）だけを消し、学習済みの `menu_essences`（我が家の好み）には影響しない。これは意図した仕様。
